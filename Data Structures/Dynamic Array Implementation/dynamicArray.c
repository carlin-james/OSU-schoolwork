...
void addHeap(DynArr *heap, TYPE val)
{
    ...
    while(cur > 0)
    {
        ...
        if(compare(getDynArr(heap, cur), getDynArr(heap, parent)) == -1)
        {
...
void removeMinHeap(DynArr *heap)
{
   ...
   heap->data[0] = heap->data[temp];
   removeAtDynArr(heap, temp);
...
void sortHeap(DynArr *heap)
{
   ...
   int tempInt = heap->size -1;

   while(tempInt > 0)
   {
       swapDynArr(heap, 0, tempInt);
       _adjustHeap(heap, tempInt, 0);
       tempInt--;
   }
