...
{
	//pointer to new task
	//use malloc, needs dynamic allocation
...
void printList(DynArr *heap)
{
    ...
    DynArr *tempDyn = createDynArr(sizeDynArr(heap));
    ...
    {
        Task *taskOne = getDynArr(heap,i);
        printf("%d   %s\n", taskOne->priority, taskOne->description);

        addHeap(tempDyn,createTask(taskOne->priority, taskOne->description));
    }

    for( i = 0; i < sizeDynArr(heap); i++)
    {
        free(getDynArr(heap, i));
        putDynArr(heap, i , getDynArr(tempDyn, i));
