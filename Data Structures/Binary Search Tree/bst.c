...
{
	//this is recursive... get base case first
    assert(val !=0);
	//base case
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    struct Node* newNode = tree->root;

    while(newNode !=0)
    {
        if(compare(val, newNode->val) == 1)

            newNode = newNode->right;

        else if(compare(val, newNode->val) == -1)
