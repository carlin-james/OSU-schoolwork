...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	struct Vertex *tempVertex = source;
	initCirListDeque(stack);
	clearVisited(g);


	addFrontCirListDeque(stack, tempVertex);


	while (!isEmptyCirListDeque(stack))
	{

		tempVertex = frontCirListDeque(stack);
...
int i;
		for (i = 0; i < tempVertex->numNeighbors; ++i)
		{
			if (!tempVertex->neighbors[i]->isVisited)
			{
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	while (!isEmptyCirListDeque(stack))
	{
    //set temp vertex = to the value of BACK for this one
		tempVertex = backCirListDeque(stack);
		//Remove from BACK on t his one
